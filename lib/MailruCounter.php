...
final class MailruCounter {
    ...
    function __construct($path) {
        ...
        $this->plugin_title = __('Wordefinery Mail.ru Counter');
        ...
        if ($this->store->site_id) {
            switch ($this->store->mode) {
                case 'widget':
                    \add_action('widgets_init', function () { register_widget('\\wordefinery\\MailruCounterWidget'); });
                    break;
                case 'footer':
    ...
    function SettingsPage() {
        include(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $this->path . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'mailrucounter-settings-page.php');
    }
        ...
        if ($r) {
            ...
            switch ($this->store->site_id_error()->error) {
                ...
                case 'response_error':
                    ...
                    break;
                case 'not_registered':
                    echo \sprintf(__('URL <code>%1$s</code> not registered.'), $url);
                    echo ' ';
                    _e('<a href="http://top.mail.ru/add" target="_blank">Register Counter</a>.');
                    break;
                default:
                    _e('Error');
                    break;
    ...
    function SettingsCheckSiteId() {
        ...
        if ($r) {
            echo \sprintf(__('Registered URL: <code>%1$s</code>'), $r);
        } else {
            echo '<span style="color:red">';
            switch ($this->store->site_id_error()->error) {
                case 'query_error':
                ...
                case 'response_error':
                    ...
                    break;
                case 'invalid_id':
                    _e('Invalid Site Identifier.');
                    echo ' ';
                    _e('<a href="http://top.mail.ru/add" target="_blank">Register Counter</a>.');
                    break;
                case 'not_registered':
                    echo \sprintf(__('Site identifier <code>%1$s</code> not registered.'), $site_id);
                    echo ' ';
                    _e('<a href="http://top.mail.ru/add" target="_blank">Register Counter</a>.');
                    break;
                default:
...
//        $urls[] = 'www.'.$h['host'].$h['path'].'/';
        \array_walk($urls, function (&$i) { $i = str_replace('//', '/', $i); });
    ...
    function SiteIdValidator($site_id) {
        if (!(isset($site_id) && $site_id + 0 > 0)) {
            throw new SettingsValidateException('error', __('Please enter a valid site identifier.'));
        } else {
            if (! $url = $this->CheckSiteId($site_id)) {
                if ($this->store->site_id_error()->error != 'query_error' && $this->store->site_id_error()->error != 'response_error' ) {
                	throw new SettingsValidateException('error', sprintf(__('Site identifier <code>%1$s</code> not registered. Please enter a valid site identifier.'), $site_id));
                } else {
                    throw new SettingsValidateException('updated', __('Site identifier validation failure.'));
                }
            } else {
                $hurl = \site_url();
                ...
                if ($url != $hurl) {
                   throw new SettingsValidateException('updated', sprintf(__('Site identifier <code>%1$s</code> registered to domain <code>%2$s</code>. This site domain <code>%3$s</code>.'), $site_id, $url, $hurl));
                }
            }
        }
    }
